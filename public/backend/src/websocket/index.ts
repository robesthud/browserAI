...
async function handleMessage(ws: WebSocket, message: any, ctx: Context) {
  ...
  switch (type) {
    ...
    // ============================================
    case 'ai:chat:stream': {
      const { requestId, messages, context, config } = payload as {
        requestId: string;
        messages: Message[];
        context?: string;
        config: AIConfig;
    ...
    // ============================================
    case 'ai:completion': {
      const { requestId, prefix, suffix, language, config } = payload as {
        requestId: string;
        prefix: string;
        suffix: string;
        language: string;
...
function broadcast(projectId: string, excludeWs: WebSocket, message: any) {
  const projectConnections = connections.get(projectId);
  if (projectConnections) {
    const msgStr = JSON.stringify(message);
    for (const ws of projectConnections) {
      if (ws !== excludeWs && (ws as any).readyState === WebSocket.OPEN) {
        (ws as any).send(msgStr);
      }
    }
  }
}
