...
export class PTYManager {
   ...
   */
  static async createSession(sessionId: string, projectId: string, ws: WebSocket): Promise<any> {
   ...
   */
  static write(sessionId: string, data: string) {
   ...
   */
  static resize(sessionId: string, cols: number, rows: number) {
    const session = this.sessions.get(sessionId);
    if (session) {
      try {
   ...
   */
  static killSession(sessionId: string) {
    const session = this.sessions.get(sessionId);
    if (session) {
