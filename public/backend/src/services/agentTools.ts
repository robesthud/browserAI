...
export class AgentTools {
   ...
   */
  static async read_file(projectId: string, path: string): Promise<string> {
    return retry(async () => {
      try {
   ...
   */
  static async write_file(projectId: string, path: string, content: string): Promise<string> {
    return retry(async () => {
      try {
   ...
   */
  static async edit_file(projectId: string, path: string, search: string, replace: string): Promise<string> {
    return retry(async () => {
      try {
   ...
   */
  static async list_dir(projectId: string, path: string): Promise<string[]> {
    return retry(async () => {
      try {
        const projectPath = join(PROJECTS_DIR, projectId, path);
   ...
   */
  static async run_command(projectId: string, command: string, cwd = ''): Promise<string> {
    return retry(async () => {
      try {
   ...
   */
  static async install_package(projectId: string, pkg: string, manager: 'npm' | 'pip'): Promise<string> {
   ...
   */
  static async browser_navigate(url: string, sessionId = 'default'): Promise<any> {
    return retry(async () => {
      return BrowserAgentService.executeAgentInstruction('go to ' + url, undefined, sessionId);
    });
  }

  static async browser_click(selector: string, sessionId = 'default'): Promise<any> {
    return retry(async () => {
      return BrowserAgentService.executeAgentInstruction('click ' + selector, undefined, sessionId);
    });
  }

  static async browser_type(selector: string, text: string, sessionId = 'default'): Promise<any> {
    return retry(async () => {
      return BrowserAgentService.executeAgentInstruction(`type "${text}" into ${selector}`, undefined, sessionId);
    });
  }

  static async browser_extract(selector: string, sessionId = 'default'): Promise<any> {
    return retry(async () => {
      return BrowserAgentService.executeAgentInstruction('extract ' + selector, undefined, sessionId);
    });
  }

  static async search_web(query: string, sessionId = 'default'): Promise<any> {
    return retry(async () => {
