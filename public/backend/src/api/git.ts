...
function getGit(projectPath: string): any {
...
export async function gitRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/clone', {
    ...
    const { url, branch, name } = request.body as {
      url: string;
      branch?: string;
      name?: string;
    };
  ...
  fastify.get('/log', {
    ...
    const { projectId, limit = 50 } = request.query as { projectId: string; limit?: number };
  ...
  fastify.post('/branch', {
    ...
    const { projectId, branch, create } = request.body as {
      projectId: string;
      branch: string;
      create?: boolean;
    };
...
async function indexDirectory(projectId: string, basePath: string, relativePath: string) {
