...
export async function figmaRoutes(fastify: FastifyInstance) {
  fastify.post('/generate-code', async (request: FastifyRequest, reply: FastifyReply) => {
    const { figmaUrl, personalToken, aiConfig } = request.body as {
      figmaUrl: string;
...
function extractSimplifiedNodes(node: any, depth = 0): any {
  if (depth > 4) return null; // Avoid extremely deep trees
  ...
  if (node.children && node.children.length > 0) {
    simplified.children = node.children
      .map((c: any) => extractSimplifiedNodes(c, depth + 1))
      .filter(Boolean)
