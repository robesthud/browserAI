...
### 🆕 Managed DeepSeek + удаление Arena bridge (v1.1.0)
- **Удалён** прежний интеграционный слой LMArenaBridge: `server/arenaCookieRefresher.js`,
  эндпоинт `/api/arena/refresh-cookie`, пресет «🌉 Arena.ai Bridge» в UI,
  переменные `ARENA_*` и `BRIDGE_*`
- **Новый модуль** `server/deepseekTokenRefresher.js` — хранит `userToken` и
