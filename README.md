...
[![Version](https://img.shields.io/badge/version-1.1.0-blue)](https://github.com/robesthud/browserAI)
...
- Production: Timeweb Cloud VPS (Ubuntu 22.04 + Docker), доступен по `APP_URL`.
- Android wrapper: `android-app/`, package: `ai.browser.app`
- Android-приложение — WebView-обёртка над production-версией. Backend/SQLite/Workspace на сервере.
- Web-изменения обновляются «по воздуху» после redeploy; APK нужно пересобирать только при изменениях в `android-app/`.
- Деплой: push в `main` → GitHub Actions `deploy-timeweb.yml` (`git pull && docker compose up -d --build`).
...
- ✨ **Gemini Web Proxy** — OpenAI-compatible proxy к Gemini Web (`http://host.docker.internal:8080/v1`) после Google-login через Playwright/noVNC
...
# открыть http://localhost  (контейнер слушает 8080, проброшен на 80)
```

### Production (Timeweb VPS + Docker)

Канонический деплой — VPS с Docker. `docker-compose.yml` поднимает сервис
`browserai` и изолированную песочницу `agent-sandbox`, используя bind-mounts
на хосте (см. `DATA_DIR` / `WORKSPACE_DIR` в `.env`):
...
Автодеплой настроен в `.github/workflows/deploy-timeweb.yml`: push в `main`
(кроме изменений только в `*.md` / `android-app/` / `cf-proxy/`) по SSH делает
`git pull && docker compose up -d --build`.

Состояние хранится в bind-mount каталогах на хосте:
- SQLite: `/data/browserai.db`
- Workspace: `/workspace`
- DeepSeek-сессия: `/data/deepseek_session.json`

> Тот же `Dockerfile` без изменений работает на любом Docker-хостинге —
> просто примонтируйте persistent volume в `/data`.
...
Для production обязательно задайте в окружении (`.env` рядом с `docker-compose.yml`):
...
Web-интерфейс обновляется автоматически после redeploy сервера.
