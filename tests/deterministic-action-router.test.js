...
describe('deterministic action router', () => {
  it('normalizes GitHub repo URLs and owner/repo slugs for download commands', () => {
    ...
    expect(extractGithubRepoUrl('скачай файлы с github robesthud/browserAI')).toBe('https://github.com/robesthud/browserAI.git')
  })
  ...
  it('routes repo download without involving the LLM', () => {
    ...
    const slugAction = routeDeterministicAction([{ role: 'user', content: 'скачай файлы с github robesthud/browserAI' }])
    expect(slugAction?.tool).toBe('git_clone')
    expect(slugAction?.args.url).toBe('https://github.com/robesthud/browserAI.git')
  })
