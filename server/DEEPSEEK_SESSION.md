...
Встроенный менеджер сессии `chat.deepseek.com`, который:
...
## Жизненный цикл токена и redeploy

Файл `deepseek_session.json` хранится **на хосте** в каталоге, который
монтируется в контейнер как `/data` (см. `DATA_DIR` в `docker-compose.yml`).

- Каждые 10 мин `heartbeat()` дёргает `GET /api/v0/users/current` и
  если в ответе пришёл `Set-Cookie` — мержит cookies в state и пишет
  файл (`saveToDisk()`).
- Каждый раз когда админ через UI/TG задаёт `userToken` или `cookies`
  через `setSession()` — тоже сохраняется на диск.
- Файл переживает: пересборку образа, `docker compose down`, реклон
  репозитория. Полный список действий — в `README.md` → «Где живёт
  токен и почему redeploy его не трогает».

## Бэкапы

`scripts/backup-deepseek-session.sh` снимает копию раз в час
(systemd timer `browserai-backup.timer`) в
`/var/backups/browserai/deepseek_session.YYYYMMDDTHHMMSSZ.json`,
ротация — `KEEP_DAYS=7`.
