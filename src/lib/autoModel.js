...
const TASK_PATTERNS = [
  {
    // Задачи, требующие выполнения tools (агент-режим): скачивание файлов,
    // работа с git/github, локальный workspace, shell, файлы. Распознаём
    // ПЕРВЫМИ — иначе слова вроде "файл" / "репозиторий" вытолкнут запрос
    // в 'chat' и пользователь получит Markdown-инструкцию вместо реального
    // вызова инструмента (см. фикс в shouldUseAgentForText).
    type: 'agent',
    keywords: [
      'скачай', 'скачать', 'загрузи', 'загрузить', 'клонируй', 'клонировать',
      'github', 'gitlab', 'gitea', 'bitbucket', 'гитхаб', 'гитлаб', 'репозиторий', 'репо ',
      'git clone', 'git pull', '.git',
      'workspace', 'воркспейс', 'рабочая область', 'в папку', 'в папке',
      'выполни команду', 'запусти команду', 'запусти скрипт', 'выполни скрипт',
      'исправь файл', 'измени файл', 'удали файл', 'переименуй файл',
      'создай файл',
    ],
    preferKeywords: ['claude', 'sonnet', 'opus', 'gpt-4', 'deepseek', 'gemini', 'qwen'],
    label: 'агент-задача',
  },
  {
    type: 'image',
